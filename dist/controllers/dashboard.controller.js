...
const getDashboard = async (req, res) => {
    try {
        ...
        const ventasHoy = await prisma_1.default.transaccion.findMany({
            where: {
        ...
        // VENTAS SEMANA
        const ventasSemana = await prisma_1.default.transaccion.findMany({
            where: {
                negocioId: negocio.id,
                tipo: 'venta',
                fecha: {
        ...
        // VENTAS MES
        const ventasMes = await prisma_1.default.transaccion.findMany({
            where: {
                negocioId: negocio.id,
                tipo: 'venta',
                fecha: {
        ...
        const proximasVisitas = await prisma_1.default.proveedor.findMany({
            where: {
                negocioId: negocio.id,
                activo: true,
                proximaVisita: {
        ...
        // ALERTAS NO LEÍDAS
        const alertasNoLeidas = await prisma_1.default.alerta.count({
            where: {
                negocioId: negocio.id,
                leida: false
            }
        ...
        const f29Actual = await prisma_1.default.declaracionF29.findUnique({
            where: {
                negocioId_mes_anio: {
                    negocioId: negocio.id,
                    mes: mesActual,
        ...
        const transaccionesConProductos = await prisma_1.default.transaccion.findMany({
            where: {
                negocioId: negocio.id,
                tipo: 'venta',
                fecha: {
...
const getReporteMensual = async (req, res) => {
    try {
        ...
        // Obtener F29
        const f29 = await prisma_1.default.declaracionF29.findUnique({
            where: {
                negocioId_mes_anio: {
                    negocioId: negocio.id,
                    mes: mesNum,
...
exports.getReporteMensual = getReporteMensual;
